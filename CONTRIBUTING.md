...
local Dockerfile. Catalog entries may pass versioned build arguments or apply
small reviewed patches under `patches/<image>/` when stale packaging inputs
cannot be overridden upstream. Patches must not change the application's
functional scope and must remain suitable for upstream contribution.
